...
public class cuenta extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        request.getRequestDispatcher("vistas/cuenta/index.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        MREJ_usuarioDAO dao = new MREJ_usuarioDAO();
        boolean success = dao.actualizar(usuario);
        if (success) {
            HttpSession session = request.getSession();
            session.setAttribute("nombre", nombre);
            session.setAttribute("password", password);
        }
        String acceso = "/cuenta";
